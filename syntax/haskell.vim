...
" things.
syn match hsFunctionList "^\s*\(\<\(where\>\|let\>\)\@![_a-z][_a-zA-Z0-9']*[[:space:]\n,]\+\)*[_a-z][_a-zA-Z0-9']*[[:space:]\n]*::" contains=hsFunction
syn match hsFunction "\s*[a-z_][a-zA-Z0-9_']*\([[:space:]\n]*\(::\|,\)\)\@=" contained
" Also support the style where the first where binding is on the same line as
" the where keyword.
syn match hsFunction "\(\<\(where\|let\)\s\+\([_a-z][_a-zA-Z0-9']*\s*,\s*\)*\)\@<=[_a-z][_a-zA-Z0-9']*\(\s*\(,\s*[_a-z][_a-zA-Z0-9']*\s*\)*::\)\@="
